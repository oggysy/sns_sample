...
import 'package:sns_sample/view/component/follow/follow_button.dart';
...
class OtherAccountPage extends ConsumerStatefulWidget {
  const OtherAccountPage({super.key, required this.account});
...
class _OtherAccountPageState extends ConsumerState<OtherAccountPage> {
  @override
  Widget build(BuildContext context) {
    final postList = ref.watch(userPostsProvider(widget.account.id));
    return Scaffold(
      appBar: AppBar(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              CircleAvatar(
                                radius: 32,
                                foregroundImage:
                                    NetworkImage(widget.account.imagePath),
                              ),
                              const SizedBox(
                                width: 10,
                              ),
                          ...
                          FollowButton(userId: widget.account.id)
                        ],
                      ),
                      const SizedBox(
                        height: 15,
                      ),
                      Text(widget.account.selfIntroduction),
                      const SizedBox(
                        height: 8,
                      ),
                      Row(
                        ...
                        children: [
                          FollowCountView(userId: widget.account.id),
                          const Text(
                            'フォロー中',
                          ...
                          FollowerCountView(userId: widget.account.id),
                          const Text(
                            'フォロワー',
                            style: TextStyle(fontSize: 9),
                          ),
